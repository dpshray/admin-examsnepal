...
const questionSchema = yup.object({
    ...
    explanation: yup.string().min(20, "Explanation must be at least 20 characters").required("Explanation is required"),
...
const TextInputField = ({
    ...
    <div className="space-y-2">
        ...
        <Input {...props} className={error ? "border-red-500 focus:border-red-500" : ""}/>
        {error && (
...
export default function ExamQuestionsPage() {
    ...
    const uploadQuestion = async (data: QuestionFormData) => {
        ...
        try {
            ...
            const response = await examService.uploadQuestion(Number(exam.id), formData)
    ...
    return (
        <main className="min-h-screen bg-gray-50 p-6 w-full">
            <header className="mb-8">
                <Button variant="ghost" onClick={handleGoBack} className="mb-4 flex items-center gap-2">
                ...
                <div className="flex items-start justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 mb-2 capitalize">{exam.exam_name}</h1>
                        <p className="text-gray-600 mb-4">{exam.description}</p>
                        <div className="flex items-center gap-4">
                            <Badge
                                variant={exam.is_active ? "default" : "secondary"}>{exam.is_active ? "Active" : "Inactive"}</Badge>
                ...
                {questions.length} question{questions.length !== 1 ? "s" : ""}
                    ...
                    <div className="flex gap-2">
                        <Button onClick={handleUploadQuestions} variant="outline" className="flex items-center gap-1">
                ...
                <section className="mb-8">
                    <Card className="w-full p-6 space-y-6">
                        <div className="space-y-2">
                            ...
                            <p className="text-muted-foreground">Create a new multiple choice question for your
                                exam.</p>
                        </div>

                        <form onSubmit={handleSubmit(uploadQuestion)} className="space-y-6" noValidate>
                            <TextInputField
                                label="Question"
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-4">
                                    <Label className="text-sm font-medium">Choose Correct Answer</Label>
                                    <RadioGroup
                                        value={correctAnswer}
                                        onValueChange={(value) =>
                                    ...
                                    >
                                        {OPTIONS.map(({value, label}) => {
                                            ...
                                            const displayValue = typeof watchedValues[key] === "string" ? watchedValues[key] : ""
                                            return (
                                                <div key={value}
                ...
                {questions.length > 0 ? (
                    <div className="space-y-6">
                        {questions.map((question, index) => (
                            <QuestionCard key={String(question.id)} question={question as any} index={index}/>
                        ))}
                        <div>
                            <CustomPagination
                    ...
                    <Card>
                        <CardContent className="text-center py-12">
                            ...
                            <p className="text-gray-600 mb-6">Upload questions or create them manually</p>
                            <div className="flex gap-2 justify-center">
                                <Button onClick={handleUploadQuestions} variant="outline"
